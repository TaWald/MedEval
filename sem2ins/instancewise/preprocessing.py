...
### Disclaimer:  This entire file is taken from nnUNet preprocessing.py ###
                ...
                for slice_id in range(shape[axis]):
                    if axis == 0:
                        ...
                        reshaped_data.append(
                            resize_fn(data[c, :, slice_id], new_shape_2d, order, cval=cval, **kwargs)
                        )
                    else:
                        reshaped_data.append(
                            resize_fn(data[c, :, :, slice_id], new_shape_2d, order, cval=cval, **kwargs)
...
def resample_patient(
    ...
    if axis is not None:
        if len(axis) == 3:
            ...
            print(
                "WARNING: axis has len 2, axis: %s, spacing: %s, target_spacing: %s"
